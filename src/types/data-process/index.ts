...
// 语音转文本配置参数
export interface SpeechToTextConfigParams {
  language: string;
...
// 创建媒体任务响应
export interface CreateMediaTaskRes {
  identifier: string;
  message: string;
}
...
// 媒体任务列表
export interface MediaTaskList {
  tasks: MediaTask[];
}

// MediaFileList 函数组件的 Props
...
export interface MediaTaskDetailMetadata {
  language: string;
  start_time: string;
  end_time: string;
  file_name: string;
...
export interface MediaTaskDetailResult {
  segments: MediaTaskDetailSegment[];
}

export interface MediaTaskDetailSegment {
  start: number;
  end: number;
  text: string;
...
export interface MediaTaskDetailWord {
  word: string;
  start: number;
  end: number;
  score: number;
